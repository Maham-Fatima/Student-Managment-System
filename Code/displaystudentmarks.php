...
if($cid){
    ...
    $sql = "SELECT DISTINCT type FROM marks";
}
...
<div class='overflow-x-auto max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6 w-full'>
    <h2 class='text-2xl font-bold mb-4'>Marks for {$cid}</h2>
";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    <h3 class='text-lg font-semibold mb-2 md:bg-blue-900 md:text-white md:text-center'>$type</h3>
...
";
    ...
    $sql = "SELECT * FROM studentmarks a JOIN student s ON a.sid = s.ID JOIN marks t ON t.mid = a.mid WHERE s.username = '{$_SESSION["username"]}' AND s.pass = '{$_SESSION["password"]}' AND t.type = '$type' AND t.cid = '$cid'";
